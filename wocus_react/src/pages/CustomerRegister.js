...
const Register = () => {
  ...
  const handleRegister = async (event) => {
    ...
    try {
      const response = await axios.post("http://localhost:8081/auth/register1", data);
      setUsername("");
      setEmail("");
      setPassword("");
      ...
      <Navbar expand="lg" className="bg-body-tertiary position-sticky top-0" style={{ zIndex: 1 }}>
        <Container>
          ...
          <Nav className="me-auto">
            <Nav.Link as={Link} to={"/customerregister"}>Register</Nav.Link>
            <Nav.Link as={Link} to={"/customerlogin"}>Login</Nav.Link>
          </Nav>
        </Container>
      </Navbar>
